...
function PolicySection({
  ...
  return (
    <section className="py-10 border-b border-ink-rim last:border-b-0">
      <div className="flex items-baseline gap-3 mb-5">
        <span className="font-mono text-xs text-fog-dim/60 tracking-widest flex-shrink-0">
          {number}
        </span>
        <span className="text-ink-rim select-none flex-shrink-0" aria-hidden="true">
          ---
        </span>
        <h2 className="font-display text-xl text-fog leading-snug">{title}</h2>
      </div>
      <div className="font-sans text-sm text-fog-dim leading-relaxed flex flex-col gap-3 pl-[4.5rem]">
        {children}
      </div>
    </section>
  );
    ...
    <ul className="list-none flex flex-col gap-2">
      {items.map((item, i) => (
        <li key={i} className="flex gap-2">
          <span className="font-mono text-cyan/50 flex-shrink-0 mt-0.5" aria-hidden="true">
            —
          </span>
          <span>{item}</span>
        </li>
...
export default function PrivacyPage() {
  return (
    <div className="min-h-screen bg-obsidian text-fog font-sans">
      {/* Top accent */}
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 font-sans text-sm text-fog-dim hover:text-cyan transition-colors duration-150 mb-10"
        >
        ...
        <header className="mb-12">
          ...
          <h1 className="font-display text-4xl text-fog leading-tight">
            Privacy Policy
          </h1>
        </header>
        ...
        <article>
          ...
          <PolicySection number="02" title="How we use it">
            <UL
              items={[
                <>
                  <strong className="font-medium text-fog">Clinical documentation only</strong>{" "}
                  — your data is used exclusively to generate SOAP notes and patient-friendly
                  summaries for your treating doctor and clinic.
                </>,
                <>
                  <strong className="font-medium text-fog">AI draft, doctor reviewed</strong>{" "}
                ...
                <>
                  <strong className="font-medium text-fog">No third-party advertising</strong>{" "}
                  — your health data is never shared with advertisers, data brokers, or
                  marketing platforms.
                </>,
                <>
                  <strong className="font-medium text-fog">Knowledge graph</strong>{" "}
          ...
          <PolicySection number="03" title="Data retention">
            <P>
              ...
              <strong className="font-medium text-fog">30 days</strong>.
          ...
          <PolicySection number="04" title="Your rights under PDPA">
            ...
            <UL
              items={[
                <>
                  <strong className="font-medium text-fog">Access</strong> — request a
                  copy of the personal data we hold about you.
                </>,
                <>
                  <strong className="font-medium text-fog">Correction</strong> — request
                  that inaccurate or incomplete data be corrected.
                </>,
                <>
                  <strong className="font-medium text-fog">Withdraw consent</strong> —
            ...
            <P>
              ...
              <strong className="font-medium text-fog">clinic administrator</strong> directly.
        ...
        <p className="font-sans text-xs text-fog-dim/60">
